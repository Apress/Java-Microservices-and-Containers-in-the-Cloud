...
public class ProductRestController {
    ...
    public ResponseEntity<Product> postProduct(@RequestBody   com.acme.ecom.product.model.Product product,    UriComponentsBuilder ucBuilder) {

    	LOGGER.info("Start");
    	LOGGER.debug("Creating Product with code: {}", product.getCode());

        List<com.acme.ecom.product.model.Product> products = productRepository.findByCode(product.getCode());
        if (products.size() > 0) {
    ...
    public ResponseEntity<Product> updateProduct(@PathVariable("id") String id, @RequestBody Product product) {
    	...
    	com.acme.ecom.product.model.Product currentProduct = productRepository.findById(id).get();
    ...
    public ResponseEntity<List<com.acme.ecom.product.model.Product>> getAllProducts() {

    	LOGGER.info("Start");
        List<com.acme.ecom.product.model.Product> products = productRepository.findAll();
        if(products.isEmpty()){
    		LOGGER.debug("No products retreived from repository");
            return new ResponseEntity<List<com.acme.ecom.product.model.Product>>(HttpStatus.NOT_FOUND);
        }
    	LOGGER.info("Ending");
        return new ResponseEntity<List<com.acme.ecom.product.model.Product>>(products, HttpStatus.OK);
    }
